...
> **更新（2026-07-03）**：门控的"永不门控"豁免从 **周日** 改成 **每周第一个交易日**（每周完整风控日）——因为周日批次已被砍掉、Tier 2 只在交易日跑，详见 [[watchy-tier2-risk-cadence]]。代码里 `_should_skip_tier2` 现在用 `is_weekly_full_risk_day(now)`（`watchy/market_calendar.py`）而非 `now.weekday()==6`。
