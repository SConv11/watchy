...
metadata: 
  ...
  modified: 2026-08-13T16:02:05.053Z
---

# Tier 2 时长/成本回归与分层 cadence（2026-08-07 定案，commit `046b1cf`）

## 🚨 2026-08-13 更新（本文下方"高峰加价已宣布未启用"已过期）

- **DeepSeek 分时计价 2026-08-16 16:00 UTC 正式上线**，平价取消。非高峰单价仍比旧平价贵，
  watchy 实际涨幅 **≈1.9×**（¥80→¥152/月）。完整价目表见 [[watchy-api-cost-baseline]]。
- **起跑时间 10:00 → `10:02` UTC**：高峰窗 06:00–10:00 UTC 的右边界官方没写含不含 10:00，
  赌错整批 ×2。两分钟是刻意留的余量（=北京 18:02，过 18:00 边界）。
- **watchlist 18 → 15 票**：删 MU / LRCX / ETN（都是值守票、且与持仓票主题重复：
  MU→SKHY(HBM)、LRCX→ASML(半导体设备)、ETN→EMR/APH/NVT(电气)）。约省 $40/年，顺带缩短批次。
- ⚠️ **本文"跑过开盘"那条因此有两个改善方向叠加**（少 3 票 + 高峰无关），但**周一仍是全量日**
  （全票 + 3 路风险辩论，且 tier2_days 对周一无效）→ 周一批次长度仍是最需要盯的。
- ⚠️ **值守票的成本地板**：周一全量豁免让**任何一只票每年至少 ~$6**，cadence 再降也降不下去，
  **只有删票能到零**。所以"降 cadence"对小仓位/低优先级票是无效杠杆。
