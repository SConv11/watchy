...
2. ~~**Gemini 开 thinking 值不值**~~ **🏁 已结案(2026-07-03，用 `scripts/compare_gemini_thinking.py --ticker MOD` A/B，不重跑管线)：不开。** 关=$0.0102(out 347)；开=$0.0347(**贵 3.4x**，thinking 2945 token=$0.0265)且**把答案截断了**(动态 thinking 吃掉 3072 预算的 2945，只剩 122 给可见答案→话没说完)、决策零改善(两边 HOLD/$197)。默认关(`gemini_thinking_budget=0`)确认正确。真要开得用固定小预算(如 512)不能用 -1，但没价值。**顺带确认 ④ digest 忠实度 OK**——关 thinking 的建议能引用 ATR/库存+48%/应收+52%/净利-34%/P/E 102x/200-SMA $197.25 等具体数字。**工具**：`scripts/compare_gemini_thinking.py`(从存档报告重放 advisor，thinking 0 vs -1，任何票都能 A/B)。
3. **DeepSeek quick 节点关 thinking**（#2，省~16%）：要给 vendored TA 的 `DeepSeekChatOpenAI` 加 `extra_body={"thinking":{"type":"disabled"}}` 开关(见 [[watchy-api-cost-baseline]] 上文 thinking 事实)，再 on/off 对比质量。最后做。
