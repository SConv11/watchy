...
2. **Gemini 开 thinking 值不值**（#1，③的 A/B）：以 MOD think=0/$0.010 为基线，secrets 临时 `gemini_thinking_budget: -1` 重跑 MOD，比 GEMINICOST(think>0 会加 $0.01–0.02) + 建议质量，然后改回。thinking 默认关是当前默认。
